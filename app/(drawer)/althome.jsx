...
const index = () => {
  ...
  const snapPoints = useMemo(() => ["7","25%", "50%", "90%"], []);
  ...
  return (
    ...
    handleComponent={() => (
      <View style={{ alignItems: 'center', padding: 10 }}>
        <Entypo name='arrow-with-circle-up' size={16} color={'#000'}/>
        <Text style={{ fontSize: 16, fontWeight: 'bold' }}>MENOY</Text>
      </View>)}
    ref={sheetRef}
    index={1}
    ...
    >
    <BottomSheetScrollView ref={catScrollRef} contentContainerStyle={styles.contentContainer}>
    { categoriesAll.map((c,index)=>
        ...
        onPress={() => {
            handleCategoryClick(c.id,c.title,index)
            handleSnapPress(0)
        }} key={index}
        >
